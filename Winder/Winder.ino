    ...
    if (strcmp(buf, "SJ") == 0) {  // set parameters
      ...
      do {
        ...
        if (strcmp(identifier, "RS") == 0) {  // Reset all variables
          ...
          // Wire size
          wait_for_serial();
          Serial.readBytes((char *)wire_size.bytes, sizeof(float));
          ...
          // Total turns
          wait_for_serial();
          Serial.readBytes((char *)turns.bytes, sizeof(float));
          ...
          // Spool length
          wait_for_serial();
          Serial.readBytes((char *)spool_length.bytes, sizeof(float));
          ...
          // Turns per layer
          wait_for_serial();
          Serial.readBytes((char *)turns_per_layer.bytes, sizeof(float));
          ...
          // turns last layer
          wait_for_serial();
          Serial.readBytes((char *)last_layer_turns.bytes, sizeof(float));
          ...
          wait_for_serial();
          Floatbyte_t tap;
          Serial.readBytes((char *)tap.bytes, sizeof(float));
...
void return_home() {
  ...
  if(shuttle.currentPosition() == 0) {
    return;
  }

  long shuttleSteps_Needed = calculateShuttleSteps(wire_size.value, turns_per_layer.value);

  long position = shuttle.currentPosition();

  if(direction == 0) {
    shuttle.move(-1 * position);
  }
  else {
    shuttle.move(shuttleSteps_Needed - position);
  }

  shuttle.setSpeed(300.0);
